...
def collect_rag_outputs(vectorstore, prompt_version: str) -> list:
    ...
    results = []
    print(f"\nRunning questions with prompt {prompt_version} ...")
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    for key in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        try:
            val = result[key]
            # In some Ragas versions, it's a list, in others it's a scalar.
            if isinstance(val, (list, tuple, np.ndarray)):
                scores[key] = float(np.mean([v for v in val if v is not None]))
            else:
...
def main():
    print("=" * 60)
    print("  Step 3: RAGAS Evaluation (OpenAI Edition)")
    print("=" * 60)
    ...
    Path(out_dir / "ragas_report.json").write_text(json.dumps(report, indent=2))
    print(f"💾 Saved {out_dir}/ragas_report.json")
